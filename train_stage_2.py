...
def train_stage_2(args):
    ...
    if args.resume:
        model.load_state_dict(torch.load(args.resume, mappping='cpu'))
    if args.num_gpus > 1:
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            print('Epoch: {}, Val Loss: {:.6}, psnr: {:.6}, ssim: {:.6}, mae'.format(epoch, np.mean(val_loss), c_psnr/(i+1), c_ssim/(i+1), c_mae/(i+1)))
            model.train()
